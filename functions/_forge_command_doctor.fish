...
function _forge_command_doctor
    set -l passed 0
    set -l warnings 0
    set -l failed 0
    ...
    if _forge_doctor_version_gte "$version" 3.4.0
        _forge_report success "Fish version $version (3.4.0+ required)"
        set passed (math $passed + 1)
    else
        _forge_report error "Fish version $version is too old; 3.4.0 or higher required"
        set failed (math $failed + 1)
    end

    if set -q _FORGE_PLUGIN_LOADED
        _forge_report success "Forge fish plugin variables are loaded"
        set passed (math $passed + 1)
    else
        _forge_report error "Forge fish plugin variables are not loaded"
        set failed (math $failed + 1)
    end
    ...
    if test -n "$_FORGE_BIN"
        if string match -rq '/' -- "$_FORGE_BIN"
            if test -x "$_FORGE_BIN"
                _forge_report success "Forge binary is configured: $_FORGE_BIN"
                set passed (math $passed + 1)
            else
                _forge_report error "Configured forge binary is not executable: $_FORGE_BIN"
                set failed (math $failed + 1)
            end
        else if command -q "$_FORGE_BIN"
            _forge_report success "Forge binary resolves from PATH: $_FORGE_BIN"
            set passed (math $passed + 1)
        else
            _forge_report error "Forge binary is not available in PATH: $_FORGE_BIN"
            _forge_doctor_detail "Installation: curl -fsSL https://forgecode.dev/cli | sh"
            set failed (math $failed + 1)
        end
    else
        _forge_report error "_FORGE_BIN is empty"
        set failed (math $failed + 1)
    end
    ...
    if test "$_FORGE_THEME_LOADED" = 1
        _forge_report success "Forge right prompt is installed"
        set passed (math $passed + 1)
    else
        _forge_report error "Forge right prompt is not installed"
        set failed (math $failed + 1)
    end
    ...
    if functions -q fish_right_prompt
        ...
        if string match -q '*__forge_status_prompt*' -- "$rp_def"
            _forge_report success "fish_right_prompt is wrapped with Forge status"
            set passed (math $passed + 1)
        else
            _forge_report warning "fish_right_prompt exists but is not wrapped by Forge"
            set warnings (math $warnings + 1)
        end
    else
        _forge_report warning "fish_right_prompt is not defined"
        set warnings (math $warnings + 1)
    end
    ...
    if command -q fzf
        ...
        if test -n "$fzf_ver"
            if _forge_doctor_version_gte "$fzf_ver" 0.36.0
                _forge_report success "fzf $fzf_ver"
            else
                _forge_report error "fzf $fzf_ver is too old; 0.36.0 or higher required"
                _forge_doctor_detail "Update: https://github.com/junegunn/fzf#installation"
                set failed (math $failed + 1)
            end
        else
            _forge_report success "fzf: installed"
        end
        set passed (math $passed + 1)
    else
        _forge_report error "fzf not found"
    ...
    if test -n "$fd_backend"; and command -q "$fd_backend"
        ...
        if test -n "$fd_ver"
            if _forge_doctor_version_gte "$fd_ver" 10.0.0
                _forge_report success "$fd_backend $fd_ver"
            else
                _forge_report error "$fd_backend $fd_ver is too old; 10.0.0 or higher required"
                _forge_doctor_detail "Update: https://github.com/sharkdp/fd#installation"
                set failed (math $failed + 1)
            end
        else
            _forge_report success "$fd_backend: installed"
        end
        set passed (math $passed + 1)
    else
        _forge_report warning "fd/fdfind not found"
    ...
    if test -n "$preview_backend"; and command -q "$preview_backend"
        ...
        if test -n "$bat_ver"
            if _forge_doctor_version_gte "$bat_ver" 0.20.0
                _forge_report success "$preview_backend $bat_ver"
            else
                _forge_report error "$preview_backend $bat_ver is too old; 0.20.0 or higher required"
                _forge_doctor_detail "Update: https://github.com/sharkdp/bat#installation"
                set failed (math $failed + 1)
            end
        else
            _forge_report success "$preview_backend: installed"
        end
        set passed (math $passed + 1)
    else
        _forge_report warning "bat not found"
    ...
    if test "$TERM_PROGRAM" = vscode
        ...
        if test -f "$vscode_settings"
            if grep -q '"terminal.integrated.macOptionIsMeta"[[:space:]]*:[[:space:]]*true' "$vscode_settings" 2>/dev/null
                ...
                _forge_report success "VS Code: Alt/Option key configured as Meta"
                set passed (math $passed + 1)
            else
                _forge_report warning "VS Code: Alt/Option key NOT configured as Meta"
            ...
            _forge_report warning "VS Code settings file not found; cannot verify Alt key configuration"
            set warnings (math $warnings + 1)
        end
    else if test "$TERM_PROGRAM" = iTerm.app
        set check_performed 1
        _forge_report info "iTerm2 detected"
        ...
        set check_performed 1
        _forge_report info "Terminal.app detected"
        ...
        if test -f "$xresources"
            if grep -q 'XTerm\*metaSendsEscape:[[:space:]]*true' "$xresources" 2>/dev/null
                ...
                _forge_report success "xterm: Meta key configured in ~/.Xresources"
                set passed (math $passed + 1)
            else
                _forge_report warning "xterm: Meta key not configured for word navigation"
                _forge_doctor_detail "Add to ~/.Xresources: XTerm*metaSendsEscape: true"
                _forge_doctor_detail "Then reload: xrdb ~/.Xresources"
                set warnings (math $warnings + 1)
            end
        else
            _forge_report info "xterm detected; no ~/.Xresources found"
    ...
    if test $check_performed -eq 0
        _forge_report info "Terminal: $terminal_info"
    ...
    if test -n "$NERD_FONT"
        if test "$NERD_FONT" = 1; or test "$NERD_FONT" = true
            _forge_report success "NERD_FONT: enabled"
            set passed (math $passed + 1)
        else
            _forge_report warning "NERD_FONT: disabled ($NERD_FONT)"
        ...
        if test "$USE_NERD_FONT" = 1; or test "$USE_NERD_FONT" = true
            _forge_report success "USE_NERD_FONT: enabled"
            set passed (math $passed + 1)
        else
            _forge_report warning "USE_NERD_FONT: disabled ($USE_NERD_FONT)"
            _forge_doctor_detail "Enable with: set -Ux NERD_FONT 1"
            set warnings (math $warnings + 1)
        end
    else
        _forge_report success "Nerd Font: enabled (default)"
    ...
    if test $failed -gt 0
        _forge_report error "Doctor finished with $failed failure(s), $warnings warning(s), $passed passing check(s)"
    else if test $warnings -gt 0
        _forge_report warning "Doctor finished with $warnings warning(s), $passed passing check(s)"
    else
        _forge_report success "Doctor finished: all $passed check(s) passed"
    end
end
