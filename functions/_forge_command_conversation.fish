function _forge_command_conversation
    ...
    if test "$input_text" = -
        if test -z "$_FORGE_PREVIOUS_CONVERSATION_ID"
            ...
            echo
            _forge_run conversation show "$_FORGE_CONVERSATION_ID"

            _forge_run conversation info "$_FORGE_CONVERSATION_ID"

            _forge_report success "Switched to conversation "(set_color --bold)"$_FORGE_CONVERSATION_ID"(set_color normal)
