function _forge_model_pick
    ...
    if test -n "$current_model"
        ...
        if test -n "$current_provider"; and test -n "$provider_field"
            set index (_forge_porcelain_find_index "$output" "$current_model" 1 "$provider_field" "$current_provider")
        else
            set index (_forge_porcelain_find_index "$output" "$current_model" 1)
        end
